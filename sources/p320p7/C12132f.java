package p320p7;

import p320p7.C12134h;

/* renamed from: p7.f */
/* compiled from: LLRBBlackValueNode */
public class C12132f<K, V> extends C12137j<K, V> {

    /* renamed from: e */
    private int f45183e = -1;

    C12132f(K k, V v, C12134h<K, V> hVar, C12134h<K, V> hVar2) {
        super(k, v, hVar, hVar2);
    }

    /* renamed from: e */
    public boolean mo36650e() {
        return false;
    }

    /* access modifiers changed from: protected */
    /* renamed from: k */
    public C12137j<K, V> mo36651k(K k, V v, C12134h<K, V> hVar, C12134h<K, V> hVar2) {
        if (k == null) {
            k = getKey();
        }
        if (v == null) {
            v = getValue();
        }
        if (hVar == null) {
            hVar = mo36655a();
        }
        if (hVar2 == null) {
            hVar2 = mo36659f();
        }
        return new C12132f(k, v, hVar, hVar2);
    }

    /* access modifiers changed from: protected */
    /* renamed from: m */
    public C12134h.C12135a mo36652m() {
        return C12134h.C12135a.BLACK;
    }

    public int size() {
        if (this.f45183e == -1) {
            this.f45183e = mo36655a().size() + 1 + mo36659f().size();
        }
        return this.f45183e;
    }

    /* access modifiers changed from: package-private */
    /* renamed from: t */
    public void mo36654t(C12134h<K, V> hVar) {
        if (this.f45183e == -1) {
            super.mo36654t(hVar);
            return;
        }
        throw new IllegalStateException("Can't set left after using size");
    }
}
